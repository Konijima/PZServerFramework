-- ChatSystem Message Router Module
...
function Module.ValidateIncomingMessage(player, data)
...
function Module.RouteLocalMessage(player, message, metadata, ack)
...
---@param player IsoPlayer
---@param message table
function Module.RouteGlobalMessage(player, message)
    local Server = ChatSystem.Server
...
function Module.RouteFactionMessage(player, message, username)
    local Server = ChatSystem.Server
...
---@param message table
---@param username string
function Module.RouteSafehouseMessage(player, message, username)
    local Server = ChatSystem.Server
...
---@param username string
---@param metadata table
function Module.RoutePrivateMessage(player, message, username, metadata)
    local Server = ChatSystem.Server
...
---@param player IsoPlayer
---@param message table
function Module.RouteAdminMessage(player, message)
    local Server = ChatSystem.Server
...
---@param player IsoPlayer
---@param message table
function Module.RouteStaffMessage(player, message)
    local Server = ChatSystem.Server
...
---@param message table
---@param metadata table
function Module.RouteRadioMessage(player, message, metadata)
    local Server = ChatSystem.Server
...
---@param metadata table
---@param ack function|nil
function Module.RouteMessage(player, channel, message, metadata, ack)
    local username = player:getUsername()

    if channel == ChatSystem.ChannelType.LOCAL then
        Module.RouteLocalMessage(player, message, metadata, ack)
        return true  -- Don't send default ack

    elseif channel == ChatSystem.ChannelType.GLOBAL then
        Module.RouteGlobalMessage(player, message)

    elseif channel == ChatSystem.ChannelType.FACTION then
        Module.RouteFactionMessage(player, message, username)

    elseif channel == ChatSystem.ChannelType.SAFEHOUSE then
        Module.RouteSafehouseMessage(player, message, username)

    elseif channel == ChatSystem.ChannelType.PRIVATE then
        Module.RoutePrivateMessage(player, message, username, metadata)

    elseif channel == ChatSystem.ChannelType.ADMIN then
        Module.RouteAdminMessage(player, message)

    elseif channel == ChatSystem.ChannelType.STAFF then
        Module.RouteStaffMessage(player, message)

    elseif channel == ChatSystem.ChannelType.RADIO then
        Module.RouteRadioMessage(player, message, metadata)
    end
